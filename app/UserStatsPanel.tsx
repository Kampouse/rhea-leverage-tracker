...
export default function UserStatsPanel({ position, onClose, allPositions }: UserStatsPanelProps) {
  ...
  return (
    <div className="fixed right-0 top-0 h-screen w-96 bg-[#0a0a0c] border-l border-cream/10 overflow-y-auto z-50">
      ...
      <button
        onClick={onClose}
        className="absolute top-4 right-4 text-taupe hover:text-cream text-2xl transition-colors z-10"
      >
        ×
      </button>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-full">
          <div className="text-taupe">Loading trader history...</div>
        </div>
      ) : userStats ? (
        <>
          {/* Header */}
          <div className="p-6 border-b border-cream/10 bg-gradient-to-b from-cream/[0.02] to-transparent">
            <div className="text-xs text-taupe mb-2 uppercase tracking-wide">Trader Profile</div>
            <div className="text-xl font-bold text-cream break-all pr-8">{truncateAddress(userStats.accountId)}</div>
            <a
          ...
          <div className="p-6 border-b border-cream/10">
            ...
            <div className={`text-4xl font-bold ${userStats.totalPnL >= 0 ? 'text-accent-green' : 'text-[#ff6b6b]'}`}>
          ...
          <div className="grid grid-cols-2 gap-px bg-cream/10 border-b border-cream/10">
            <div className="bg-[#0a0a0c] p-4">
              ...
              <div className="text-2xl font-bold text-lime">{userStats.winRate.toFixed(1)}%</div>
            ...
            <div className="bg-[#0a0a0c] p-4">
              ...
              <div className="text-2xl font-bold text-cream">{userStats.totalTrades}</div>
            ...
            <div className="bg-[#0a0a0c] p-4">
              ...
              <div className="text-2xl font-bold text-cream">{userStats.avgLeverage.toFixed(1)}x</div>
            </div>
            <div className="bg-[#0a0a0c] p-4">
              <div className="text-xs text-taupe mb-1 uppercase tracking-wide">Total Volume</div>
              <div className="text-2xl font-bold text-cream">{formatCurrency(userStats.totalVolume)}</div>
            </div>
          </div>
          ...
          {userStats.activePositions.length > 0 && (
            <div className="p-6 border-b border-cream/10">
              <h3 className="text-sm font-semibold text-cream mb-4 uppercase tracking-wide">
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between py-2 px-3 bg-cream/[0.02] rounded border border-cream/5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-cream">{tokenSymbol}</span>
                      </div>
                      <span className={`text-sm font-semibold ${pnl >= 0 ? 'text-accent-green' : 'text-[#ff6b6b]'}`}>
